...
+ (CGSize)sizeForText:(NSString*)text
{
    ...
    if ([[[UIDevice currentDevice] systemVersion] floatValue] < 7.0) {
        textSize = [text sizeWithFont:font
                    constrainedToSize:(CGSize){WrapWidth, CGFLOAT_MAX}];
    }else {
        textSize = [text boundingRectWithSize:CGSizeMake(WrapWidth, 9999) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:font} context:nil].size;
    }
...
- (void)drawRect:(CGRect)rect
{
    ...
    if ([[[UIDevice currentDevice] systemVersion] floatValue] < 7.0) {
        ...
        [_text drawInRect:textRect withAttributes:@{NSFontAttributeName:font}];
    }
