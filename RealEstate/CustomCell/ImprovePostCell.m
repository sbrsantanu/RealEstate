...
@implementation ImprovePostCell
...
- (void)scrollToNext
{
    ...
    for (int i = 0; i < 3; i++) {
        AsyncImageView *imageView = [[AsyncImageView alloc] initWithFrame:CGRectMake(0,0,bounds.size.width,180)];
        imageView.userInteractionEnabled = YES;
        if (type == 1) {
            [imageView setImage:[UIImage imageWithData:[curImages objectAtIndex:i]]];
        }else {
            [imageView setImageURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",propertyImageUrl,[curImages objectAtIndex:i]]]];
        }
...
- (void)refreshScrollView {
    ...
    self.lblImgNumber.text = [NSString stringWithFormat:@"%ld / %ld",curPage, totalPage];
    if (nIndex <= 1) {
        [self.imgView setContentOffset:CGPointMake(0,0)];
    }else {
        [self.imgView setContentOffset:CGPointMake(bounds.size.width,0)];
    }
}
...
- (void) setPropertyDetail:(int)pid pictures:(NSArray *)pictureArray type:(int)imageType {
    ...
    if (totalPage <= 1) {
        self.imgView.contentSize = CGSizeMake(bounds.size.width, 180);
    }else {
        self.imgView.contentSize = CGSizeMake(bounds.size.width * 3,180);
    }
